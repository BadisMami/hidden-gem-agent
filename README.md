...
company's `platform` (`greenhouse` or `custom`) and, for Greenhouse,
`platform_identifier` (the board token). `internship_monitor_service.py`
...
- **Custom company career pages** — a best-effort link-scraper
  ...
  normalized internship records). Companies on `platform=custom` are
  reported as "skipped (no adapter)" in the monitor summary until this is
  built out.
- **Lever, Workday** — planned, not yet implemented (`lever_monitor.py`,
  `workday_monitor.py` are empty stubs).
...
1. Wire `custom_monitor.py` output into a normalized-internship adapter so
   `platform=custom` companies flow into SQLite like Greenhouse does.
